...
def structure_trading_pairs(pairs, limit):
    ...
    for pair_a in pairs_list:
        ...
        a_token_0_id = pair_a["token0"]["id"]  # smart contract address
        a_token_1_id = pair_a["token1"]["id"]
        a_contract = pair_a["id"]  # pool address
        ...
        # print(a_pair, a_contract, a_token_1_decimals, a_token_1_price)

        # Put (A) into box for matching with (B)
        ...
        for pair_b in pairs_list:
            ...
            b_token_0_id = pair_b["token0"]["id"]  # smart contract address
            b_token_1_id = pair_b["token1"]["id"]
            b_contract = pair_b["id"]  # pool address
            ...
            if a_pair != b_pair:
                if b_base in a_pair_box or b_quote in a_pair_box:
                    ...
                    for pair_c in pairs_list:
                        ...
                        c_contract = pair_c["id"]  # pool address
                        ...
                        if c_pair != a_pair and c_pair != b_pair:
                            ...
                            if counts_c_base == 2 and counts_c_quote == 2 and c_base != c_quote:
                                ...
                                if unique_string not in remove_duplicates_list:
                                    ...
                                    remove_duplicates_list.append(
                                        unique_string)
    print(f"{len(triangular_pairs_list)} tradeable pairs found")
...
def calc_triangular_arb_surface_rate(t_pair, min_rate):
    ...
    for direction in direction_list:
        ...
        if direction == "forward":
            swap_1 = a_base
            swap_2 = a_quote
            # in uniswap no need to invert anything or deal with bid/ask
            swap_1_rate = a_token_1_price
            pool_direction_trade_1 = "baseToQuote"

        # Assume start with aQuote if reverse
        ...
        if direction == "forward":
            if a_quote == b_quote and calculated == 0:
                swap_2_rate = b_token_0_price  # <-- the price of token0 expressed in units of token1
